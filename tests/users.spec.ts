...
import { UsersPage } from '../src/users.page';
...
test.describe('Usuários', () => {
  test('USR-03: Valida a funcionalidade de busca e atualização da tabela.', async () => {
    await usersPage.searchUser('Nora Jones');
    const userRow = await usersPage.verifyUserVisible('Nora Jones');
  ...
  test('USR-05: Valida a reordenação por clique nos cabeçalhos da tabela.', async () => {
    ...
    const userRow = await usersPage.verifyUserVisible('Alice Johnson');
    const userRow2 = await usersPage.verifyUserVisible('Oscar Wilde');
  ...
  test('USR-08: Valida a funcionalidade de deletar um usuário na tabela.', async () => {
    const deletedUserRow = await usersPage.deleteUser('Bob Smith', '2');
